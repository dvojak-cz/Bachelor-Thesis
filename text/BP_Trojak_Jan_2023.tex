...
This thesis presents the possibility of extending the kubernetes system with the mentioned functionalities. This method allows communication with devices in private networks using TCP and UDP protocols. The presented solution offers flexibility of use and does not present any limitation restricting standard use. The solution is implemented using established standards.  
...
\ctufitkeywordsCZE{Kubernetes, CNI, network~driver, privátní~síťový~segment, edge~cloud~computing, Kubernetes operator, síťování, TCP, UDP, K8S}
...
\input{text/others/seznam_zkratek}
\chapter{Seznam pojmů}
\input{text/others/seznam_pojmu}
%%%%%%%%%%%%%%%%%%%
% ABBREVIATIONS END
%%%%%%%%%%%%%%%%%%%
...
\include{text/others/medium} % include `medium.tex' from `text/' subdirectory
\end{document}
